...
sampler transitionSampler
{
  ...
  '------------------*/
/*
These effects run when SweetFX is initialized and disable themselves when a counter (RFX_timeleft) reaches 0
*/
...
void FadeIn(in float4 position : SV_Position, in float2 texcoord : TEXCOORD0, out float4 color : SV_Target)
{
	...
	color.rgb *= -RFX_timeleft * (1.0 / 8000.0) + 1.0;
}
...
{
		const float curtain_time = 3500.0; //Time it takes for the curtain to slide away. (RFX_timeleft - curtain_time) will be the time before it starts to slide away.

		float coord = abs(texcoord.x - 0.5);
		float factor = saturate(1.0 - RFX_timeleft / curtain_time);
...
{
  float coord = abs(texcoord.x - 0.5);
  float factor = (RFX_timeleft / 8000.0);
  if (coord < factor)
	  color = tex2D(RFX_backbufferColor, texcoord);
  else
	  color = tex2D(transitionSampler, texcoord + float2(texcoord.x < 0.5 ? factor : -factor, 0));
...
void ImageFadeOut(in float4 position : SV_Position, in float2 texcoord : TEXCOORD0, out float3 color : SV_Target)
{
	...
	color = lerp(color,image, saturate(RFX_timeleft * (1.0 / 1000.0)));
}

technique Transition_Tech < bool enabled = true; int timeout = Transition_time; int toggle = Transition_ToggleKey;> //sets the RFX_timeleft value. When it reaches 0 the technique is disabled.
{
	pass
	{
